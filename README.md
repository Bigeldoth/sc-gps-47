...
- **Capture region**: width and right margin scale with the capture monitor's
  horizontal resolution (600 px wide with a 10 px right margin at 1920 px;
  800 px wide with a 13 px margin at 2560 px). Height stays at 60 px from the
  top edge. Screenshot test mode uses the same geometry unless a custom crop is set.
