...
3. The Inno Setup wizard installs SpaceDrive into `C:\Program Files\SpaceDrive\`. If Tesseract OCR is not already present, it is automatically downloaded from UB-Mannheim and installed silently — no extra step on your side.
4. Launch *SpaceDrive GPS* from the Start menu.

Tesseract + the ONNX glyph classifier work out of the box. **PaddleOCR** (CPU / GPU) is optional and can be added later from *Options → Manage engines…*; the Engine Manager will also auto-download Python 3.12 if your machine doesn't have it.
...
- Optional **PaddleOCR**: installed on demand from *Options → Manage engines…* into a dedicated `.venv-paddle/` (Python 3.12) — see [`docs/BUILD.md`](docs/BUILD.md)
